...
public class AudioStreamStressTest : MonoBehaviour
{
    ...
    /// </summary>
    readonly List<AudioStream.AudioStream> audioStreams = new List<AudioStream.AudioStream>();
    ...
    IEnumerator Start()
    {
        ...
        {
            var go = new GameObject("AudioStream#" + i);

            var @as = go.AddComponent<AudioStream.AudioStream>();
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("Stress testing scene for the AudioStream component", AudioStreamMainScene.guiStyleLabelNormal);
        GUILayout.Label(string.Format("{0} AudioStream game objects are created at Start and are set to start streaming simultaneously the same url", this.audioStreams.Count), AudioStreamMainScene.guiStyleLabelNormal);
